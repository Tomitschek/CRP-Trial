...
def analyze_data(df):
    ...
    plt.savefig('./output/crp_over_time.png')
    plt.close()
    ...
    g.savefig('./output/individual_patient_plots.png')
    plt.close()
    ...
    plt.ylabel('CRP-Wert (mg/L)')
    plt.legend(title='Gruppe', labels=['Kontrolle', 'Behandelt'])
    plt.savefig('./output/crp_boxplot.png')
    plt.close()
...
def render_to_markdown(results, output_filepath):
    ...
    os.makedirs(os.path.dirname(output_filepath) or '.', exist_okay=True)
    ...
    with open(output_filepath, 'w', encoding='utf-8') as f:
        ...
        f.write("\n## Abbildungen:\n")
        f.write("![CRP-Verläufe über die Zeit nach Gruppe](./output/crp_over_time.png)\n")
        f.write("![Individuelle Patientenverläufe](./output/individual_patient_plots.png)\n")
        f.write("![CRP-Werte nach Tag und Gruppe](./output/crp_boxplot.png)\n")

if __name__ == "__main__":
    df = load_data('crp_raw_data.csv')
    results = analyze_data(df)
    render_to_markdown(results, './output/crp_analysis_results.md')